...
{
 int i;
 int toppos=GetSystemMetrics(SM_CYSCREEN)-30;

 //Отображаем окно
 ShowWindow(hWnd, SW_SHOW);
 //Установить верхнюю позицию окна в левый нижний угол экрана.
 SetWindowPos(hWnd, HWND_TOPMOST, -4, toppos, 96, 32, SWP_SHOWWINDOW);
 UpdateWindow(hWnd);
 ...
 {
		toppos=toppos-4;
		SetWindowPos(hWnd, HWND_TOPMOST, 0, toppos, 96, 30, SWP_SHOWWINDOW);
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_PAINT:
		...
		BitBlt(hdc, 0, 0, 96, 30, hdcBits, 0, 0, SRCCOPY);
		DeleteDC(hdcBits);
		EndPaint(hWnd, &ps);
		break;
	case WM_DESTROY:
